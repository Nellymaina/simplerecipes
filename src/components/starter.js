...
export default function Cuisines() {
  ...
  return (
    ...
    <select id="sort" class="border border-gray-300 p-2 rounded bg-white shadow-md" value={sortOrder} onChange={(e) => setSortOrder(e.target.value)}>
      ...
      {filteredRecipes.filter(item=>item.category==="Starter").map((item, index) => (
                    <div key={index} className="w-auto border-1 border-gray-100 shadow-mdb rounded-2xl shadow-gray-100">
              ...
              <p className="text-[12px] sm:text-[13px] text-blue-400 font-medium"> {item.duration}</p>
                        ...
                        <div className='p-2 h-auto flex justify-center align-center'><p className="text-[14px] sm:text-[16px]">{item.name}</p></div>
