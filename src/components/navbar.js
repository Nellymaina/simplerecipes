...
export default function Navbar() {
  ...
  return (
    <div className="fixed top-0 left-0 w-full bg-black h-auto px-2 py-6 sm:p-3 flex items-center justify-between z-50">
      ...
      >
        {toggle ? <CloseIcon fontSize="medium" className="text-red-600 border-none" /> : <MenuIcon fontSize="medium" className='text-blue-600 border-none md:hidden' />}
      </motion.button>

      {/* Logo - Centered on Mobile, Left on Larger Screens */}
      <div className="md:hidden absolute left-1/2 transform -translate-x-1/2">
        <Link to='/'><img src={image} alt="Logo" className="h-[7vh] sm:h-[11vh]" /></Link>
      </div>
      <div className="hidden md:block">
        <Link to='/'><img src={image} alt="Logo" className="h-[11vh]" /></Link>
      </div>
      ...
      <motion.ul
        ...
        className={`absolute top-full md:hidden left-0 w-full bg-black text-white flex flex-col items-center py-4 space-y-4 transition-all duration-300 text-[14px] md:text-[16px] ${
          toggle ? "block" : "hidden"
...
<NavLink to="/" className={({ isActive }) => `pr-4 ${isActive ? "text-blue-600 font-bold" : "text-white"}`}>Home</NavLink>
      ...
      <Link to='/login'><button className='hidden md:flex w-auto px-6 py-1 rounded-lg bg-red-600 text-white underline hover:bg-blue-600'>login</button></Link>
